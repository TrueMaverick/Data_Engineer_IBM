...
hour=$(TZ='Morocco/Casablanca' date -u +%NH) 
...
obs_tmp=$(cat -A temperatures.txt | head -1 |  cut -d "+" -f2 | cut -d "^" -f5 | cut -d "m" -f2)
...
# creating weather report log file (only run the first time when creating the file on terminal)
